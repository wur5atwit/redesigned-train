...
def run_conflictChecker(file_listbox):
    if "Student Exam File" in files and "Room Capacities File" in files:
        try:
            ...
            student_conflicts, students_with_conflicts = ConflictChecker2.count_student_conflicts(possible_schedule_file)
            room_conflicts, conflict_details = ConflictChecker2.count_room_conflicts(room_capacities_file, possible_schedule_file)
            students_with_multiple_exams_count, students_with_multiple_exams_details = ConflictChecker2.count_students_with_multiple_exams(possible_schedule_file)
            double_booked_rooms_count, double_booked_rooms_details = ConflictChecker2.count_double_booked_rooms(possible_schedule_file)
...
tk.Button(main_frame, text="File Merger", command=lambda: raise_frame(function5_frame), bg=button_color).pack(pady=10)
...
tk.Label(function2_frame, text="Schedule Conflicts Checker requires the 'Student Exam File', and 'Room Capacities File'").pack()
file_listbox2 = Listbox(function2_frame, selectmode=EXTENDED, width=100, height=10)
file_listbox2.pack(pady=20)
tk.Button(function2_frame, text="Upload Student Exam File", command=lambda: add_file('Student Exam File', file_listbox2)).pack()
...
tk.Label(function5_frame, text="Combines 'Student' and 'Exam' files together").pack()
